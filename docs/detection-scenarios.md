...
중앙값 4 / 상위10% 6 / 최대 170
```
- **20은 자연 골짜기(11-19=14개)에 위치** → 자의적 아님. 사람 행동(1~5회)과 명확히 분리 → **오탐 ~0**(정상행동 안 잡힘).
- 97% IP가 peak ≤10(저속 정찰), 별개 덩어리 20+(34개)=공격적 자동봇 → 둘을 깔끔히 가름.
- **한계(정직)**: 20/5분은 *"시끄러운 무차별 대입"(34 IP)*만 잡음. 저속(3~10회) ~1650 IP도 공격이나 통과 → **별도 "저속/account-spraying" 룰**(하루 실패 N회+ / 여러 계정 시도) 저심각도로 보완해야 완성.
