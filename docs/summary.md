...
- **탐지 재배치(2026-07-01)**: 버스트 알림(20/5분)은 fail2ban 차단(5/5분)과 중복이라 **은퇴** → 탐지를 *차단이 못 보는 축*으로 이동. 성공 로그인(침해) + **저속·계정 스프레이(≥10 실패/12h, 실측상 한 IP 30계정·peak1)**. "inline 차단 도입 시 higher-threshold 탐지 알림은 구조적 중복"이라는 실증.
