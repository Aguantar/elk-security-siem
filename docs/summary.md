...
- **탐지 재배치(2026-07-01)**: 버스트 알림(20/5분)은 fail2ban 차단(5/5분)과 중복이라 **비활성화** → 탐지를 *차단이 못 보는 축*으로 이동. 성공 로그인(침해) + **저속·계정 스프레이(≥10 실패/12h, 실측상 한 IP 30계정·peak1)**. "inline 차단 도입 시 higher-threshold 탐지 알림은 구조적 중복"이라는 실증.
- 자동 차단(fail2ban) **적용** — SSH 5분 5회→IP 차단 + 허용목록 lockout 방지. 알림(20/5분)보다 공격적인 5/5분 — 차단은 시끄럽지 않고 내 IP는 예외라 더 빨리 막음("알림으로 받을 가치"≠"차단할 가치", 임계값 의도적 분리). (차단은 IP/CIDR 기준, 지오는 분석용)
